...
export class Orchestra {
  ...
  async processInput(userInput: string): Promise<AIResponse> {
    try {
      ...
      // Check for meditation creation pattern: "add note /" followed by text
      const meditationPattern = /^add note\s*\/\s*(.+)$/i;
      const meditationMatch = userInput.match(meditationPattern);
      if (meditationMatch) {
        const meditationText = meditationMatch[1].trim();
        if (meditationText) {
          return await this.createMeditation(meditationText);
        }
      }
  ...
  private recognizeNavigationAction(userInput: string): { type: 'navigate'; target: string; data?: any } | null {
    ...
    const navigationPatterns = [
      ...
      // Meditations patterns
      { pattern: /^(meditations|meditation|meditations list|meditation list)$/, target: 'meditations-list' },
      { pattern: /^(open meditations|open meditation list|show meditations|go to meditations|navigate to meditations)$/, target: 'meditations-list' },
      { pattern: /^(view meditations|list meditations|browse meditations)$/, target: 'meditations-list' }
    ];
  ...
  /**
   * Create a meditation from text (for "add note /" pattern)
   */
  private async createMeditation(text: string): Promise<AIResponse> {
    try {
      console.log('Creating meditation with text:', text);
      
      // Create meditation (ID will be auto-generated as {number}-{DDMMYYYY})
      const now = Timestamp.now();
      const meditationData: Omit<MeditationData, 'id'> = {
        text: text,
        createdAt: now,
        updatedAt: now
      };
      ...
      if (result.success) {
        return {
          success: true,
          message: `Meditation created successfully: "${text}"`
        };
      } else {
        return {
          success: false,
          message: `Failed to create meditation: ${result.error}`,
          error: result.error
        };
      }
    } catch (error) {
      ...
      return {
        success: false,
        message: `Error creating meditation: ${error instanceof Error ? error.message : 'Unknown error'}`,
        error: error instanceof Error ? error.message : 'Unknown error'
      };
    }
  }
