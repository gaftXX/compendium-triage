...
export interface ClaudeChatWithToolsResponse {
  content: Array<{
    type: 'text' | 'tool_use';
    text?: string;
    id?: string;
...
export class ClaudeAIService {
  ...
  /**
   * Chat with Claude AI with tool-calling support (Gen 2 Orchestra)
   */
  public async chatWithTools(
  ...
  ): Promise<ClaudeChatWithToolsResponse> {
    const keyToUse = apiKey || this.apiKey;
    
    if (!keyToUse) {
      throw new Error('Claude API key is required for tool-calling');
    }
    ...
    try {
      ...
      console.error('Claude AI tool-calling error:', error);
      throw new Error('Failed to chat with tools: ' + (error as Error).message);
    }
  }
