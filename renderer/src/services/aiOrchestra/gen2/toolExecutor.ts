...
import { getOfficeFullProfile } from '../../data/officeProfileService';
...
export class ToolExecutor {
  ...
  private constructor() {
    ...
    this.noteProcessing = new NoteProcessing();
  }
  ...
  private async executeNoteSystemTool(toolName: string, input: Record<string, any>): Promise<ToolExecutionResult> {
    switch (toolName) {
      case 'activate_note_system': {
        ...
        try {
          await this.noteProcessing.processNote(unstructuredText);
  ...
  private async executeSystemTool(toolName: string, input: Record<string, any>): Promise<ToolExecutionResult> {
    switch (toolName) {
